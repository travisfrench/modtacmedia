...
// Deterministic RNG so layout stays stable
...
/**
 * ScrollImageOverlay
 * - Visuals loop forever (modulo translation)
 * - Scroll driver never ends (we extend section height as you near the bottom)
 * - Sparse, un-angled images that drift through the viewport with scroll
 * - Lightbox is portaled to body so it always overlays
 */
export function ScrollImageOverlay({
  ...
  // ----- Visual loop span -----
  // IMPORTANT: the visual span should NOT change when we extend the driver height.
  // Otherwise you’ll feel jumps. So we base it only on the *initial* heightVh.
  ...
  React.useEffect(() => {
    ...
    const update = () => {
      ...
      // only drive while in section
      const minY = sectionTop;
      const maxY = sectionTop + sectionHeight - viewH;
      const y = window.scrollY;
      if (y < minY || y > maxY) return;

      const raw = y - sectionTop; // distance into section
      const span = travelY;

      // modulo wrap for infinite visual loop
          ...
          <motion.div style={{ y: loopY }} className="relative h-[1px] w-full will-change-transform">
            {tiled.map((p) => (
              <motion.button
                ...
                className="absolute pointer-events-auto overflow-hidden rounded-2xl border border-white/10 bg-black/20 shadow-[0_18px_60px_rgba(0,0,0,0.55)] outline-none"
                style={{
                  left: p.x,
                  top: p.y,
                  width: p.w,
